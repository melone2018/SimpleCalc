...
public class MainActivity extends Activity {
    ...
    public boolean checkOperands(){
        boolean checkFirst = TextUtils.isEmpty(mOperandOneEditText.getText());
        boolean checkSecond  = TextUtils.isEmpty(mOperandTwoEditText.getText().toString());
        return checkFirst || checkSecond;
    }
    private void compute(Calculator.Operator operator) {
        double operandOne;
        double operandTwo;
        boolean illegal = checkOperands();
        if(illegal){
        ...
        try {
            ...
            if(operandTwo==0){
                Toast.makeText(this, "Divied by Zero", Toast.LENGTH_SHORT).show();
                return;
            }
